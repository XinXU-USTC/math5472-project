...
class GaussianDiffusion:
    ...
    def x0_sample(self,
                      ...
                      timestep):
        """
        The function used for sampling x0.
        """ 
        img = x_start
        ...
        #pbar = tqdm(list(range(timestep))[::-1])
        time = timestep
        for idx in range(5):
    ...
    def dpm_sample(self,
                   ...
                   timestep):
        """
        The function to predict x_0 using dpm_solver
        """
        from .dpm_solver import NoiseScheduleVP, model_wrapper, DPM_Solver

        noise_schedule = NoiseScheduleVP(schedule='discrete', betas=torch.Tensor(self.betas))
        img = x_start
        def model_aux(x, t):
        ...
        if timestep > 4:
            x_sample = dpm_solver.sample(
                            img,
                            t_start=(timestep + 1)/self.num_timesteps,
                            steps=5,
                            order=3,
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            # predict xstart_hat
            #def fn(img):
            #    return self.x0_sample(model, img, time)
            #fn_x0 = partial(self.x0_sample, model = model, timestep = time)
            #x_0_hat =self.x0_sample(model, img, time)
            #x_0_hat = checkpoint.checkpoint(fn, img)
            #x_0_hat = out['pred_xstart']
            #x_0_hat = self.dpm_sample(model, img, idx)
            if(idx > 0):
                x_0_hat = self.dpm_sample(model, img, idx)
            else:
                x_0_hat = out['pred_xstart']
