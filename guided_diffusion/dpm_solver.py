...
def model_wrapper(
    ...
    def compute_x_0_hat(x_prev, t_continuous, noise):
        alpha_t, sigma_t = noise_schedule.marginal_alpha(t_continuous), noise_schedule.marginal_std(t_continuous)
        x_0_hat = (x_prev - sigma_t * noise) / alpha_t
        return x_0_hat

    #def cond_dps_grad(x_0_hat, measurement, x_prev, operator, **kwargs):
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
        ...
        elif guidance_type == "dps":
            assert classifier_fn is not None
            assert condition is not None
            # classifier_fn acts as operator and condition acts as the measurement
            with torch.enable_grad():
                x_in = x.detach().requires_grad_(True)
                noise = noise_pred_fn(x_in, t_continuous)
                x_0_hat = compute_x_0_hat(x_in, t_continuous, noise)
                neg_norm_grad, _ = classifier_fn(
            ...
            sigma_t = noise_schedule.marginal_std(t_continuous)
            norm_grad = -neg_norm_grad
            return noise - sigma_t * norm_grad

    assert model_type in ["noise", "x_start", "v"]
    assert guidance_type in ["uncond", "classifier", "classifier-free", "dps"]
    return model_fn
