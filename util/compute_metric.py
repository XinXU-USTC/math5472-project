...
label_root = Path(f'/home/mathxuxin/diffusion-posterior-sampling/exp/aaa/50/{task}/label')

delta_recon_root = Path(f'/home/mathxuxin/diffusion-posterior-sampling/exp/aaa/50/{task}/recon')
...
for idx in tqdm(range(3)):
    ...
    ssim_delta = structural_similarity(label, delta_recon, multichannel=True)
    ...
    delta_recon = delta_recon.view(1, 3, 32, 32) * 2. - 1.
    #normal_recon = normal_recon.view(1, 3, 256, 256) * 2. - 1.
    label = label.view(1, 3, 32, 32) * 2. - 1.
...
ssim_delta_avg = sum(ssim_list) / len(ssim_list)
