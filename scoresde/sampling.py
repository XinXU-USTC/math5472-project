...
from scoresde.models.utils import from_flattened_numpy, to_flattened_numpy, get_score_fn, get_noise_fn
from scipy import integrate
import scoresde.sde_lib as sde_lib
...
class EulerMaruyamaPredictor(Predictor):
  ...
  def update_fn(self, x, t):
    ...
    drift, diffusion, score = self.rsde.sde(x, t)
    x_mean = x + drift * dt
    x = x_mean + diffusion[:, None, None, None] * np.sqrt(-dt) * z
    # add predicting x_start
    #mean, std = self.sde.marginal_coef(t+dt)
    #s = self.score_fn(x, t+dt)
    #x_start = (x+std*std*s)/mean
